...
class Server{
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath){
		...
		try{
			...
			$this->logger->info("Loading GenisysPlus.yml...");
			if(!file_exists($this->dataPath . "Genisys.yml")){
				$content = file_get_contents($this->filePath . "src/pocketmine/resources/GenisysPlus.yml");
				if($version->isDev()){
					$content = str_replace("preferred-channel: stable", "preferred-channel: beta", $content);
				}
				@file_put_contents($this->dataPath . "GenisysPlus.yml", $content);
			}
			$this->config = new Config($this->dataPath . "GenisysPlus.yml", Config::YAML, []);
